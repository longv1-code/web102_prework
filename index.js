...
let totalAmountRaised = GAMES_JSON.reduce( (acc, game) => {
...
let countUnfunded = GAMES_JSON.reduce( (sum, game) => {
    return game.goal > game.pledged ? sum + 1 : sum;
}, 0)

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalAmountRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length == 1 ? "game" : "games"}.
                    Currrently, ${countUnfunded} ${countUnfunded == 1 ? "game" : "games"} remains unfunded.
